...
# This file shouldn't be sourced directly; instead, the top-level `.rc` file sources it.
#
...
# a user's `.zshrc`. (Instead, `source` the top-level `.rc` file if needed.)
#####
...
emulate sh -c '. "${SampShell_ROOTDIR:?}/.rc"'
...
# HISTFILE=...     # HISTFILE is already setup within `posix/.rc`.
