...
# Used to create widgets. Usage is `widget-function '^Q' && function $REPLY { ... }`
function widget-function {
	bindkey $1 ${2-SampShell-widget-$1} && zle -N $_ && REPLY=$_
}

# If a region is currently active, then deletes the region. Otherwise, deletes a character
# backwards. The point is to let delete (ie `^?`) delete regions (which can be set via `^@`, ie
# ctrl+space) if they are active.
widget-function '^?' && function $REPLY {
...
widget-function '^[/' && function $REPLY {
	...
	zle backward-kill-word && zle -f kill
}
...
# For some reason `backward-{zap,delete}-to-char` don't exist, so we gotta do our own
widget-function '^[=' backward-zap-to-char && function $REPLY {
	 zle zap-to-char -n $(( - ${NUMERIC:-1} ))
}

widget-function '^[=' backward-delete-to-char && function $REPLY {
